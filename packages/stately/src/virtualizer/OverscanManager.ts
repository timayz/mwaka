...
export class OverscanManager {
  ...
  collectMetrics() {
    ...
    if (this.visibleRect.height > 0) {
      const o = Math.abs(this.velocity.y * (this.averageTime.value + this.averagePerf.value));
      this.overscanY.addSample(o);
    }

    if (this.visibleRect.width > 0) {
      const o = Math.abs(this.velocity.x * (this.averageTime.value + this.averagePerf.value));
  ...
  getOverscannedRect() {
    ...
    const overscanY =
      Math.round(Math.min(this.visibleRect.height * 2, this.overscanY.value) / 100) * 100;
    ...
    const overscanX =
      Math.round(Math.min(this.visibleRect.width * 2, this.overscanX.value) / 100) * 100;
